...
export function lefongTrap(moves: PgnMove[]): TrophyCheckResult {
  ...
  if (
    !moves
      .slice(0, 10 * 2)
      .some((move) => ['Bxg7', 'Bxg2', 'Bxb7', 'Bxb2'].includes(move.notation.notation))
  ) {
    return [];
  }
  ...
  for (let moveNum in chessJsMoves) {
    ...
    for (let sequence of lefongSequences) {
      if (
        ...
        (chessJsMoves[moveNumber + 3] === undefined ||
          chessJsMoves[moveNumber + 3].to !== chessJsMoves[moveNumber + 2].to)
      ) {
        return [
          {
    ...
    for (let sequence of lefongSequences) {
      if (
        ...
        (chessJsMoves[moveNumber + 4] === undefined ||
          chessJsMoves[moveNumber + 4].to !== chessJsMoves[moveNumber + 3].to)
      ) {
        return [
          {
