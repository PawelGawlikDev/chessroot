...
export function doublePawnDiamond(fen: string): TrophyCheckResult {
  ...
  for (let color of ['P', 'p']) {
    const regex = new RegExp(
      `(?=${color}([A-Za-z.]{6})${color}([A-Za-z.])${color}([A-Za-z.]{6})${color})`,
      'g',
    );
...
export function connectEightOnRank(fen: string, rank: number): TrophyCheckResult {
  ...
  if (position.substring(startingSquareForWhite, startingSquareForWhite + 8) === 'PPPPPPPP') {
    ...
    position.substring(startingSquareForBlack, startingSquareForBlack + 8) === 'pppppppp'
  ) {
    result.push({
      color: 'b',
    });
...
export function pawnTrapezoid(fen: string): TrophyCheckResult {
  ...
  match = /PP([A-Za-z.]{5})P([A-Za-z.]{2})P([A-Za-z.]{3})P([A-Za-z.]{4})P/.exec(
    position.substring(0, 40),
  );
  ...
  match = /p([A-Za-z.]{4})p([A-Za-z.]{3})p([A-Za-z.]{2})p([A-Za-z.]{5})pp/.exec(
    position.substring(24, 64),
  );
