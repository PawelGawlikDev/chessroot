...
export function pawnCheckmate(moves: PgnMove[]): TrophyCheckResult {
  ...
  if (
    !lastMove.notation.fig &&
    !lastMove.notation.promotion &&
    lastMove.notation.check === '#' &&
    !lastMove.notation.notation.includes('O-O')
  ) {
    return [
      {
        color: lastMove.turn,
...
export function g5mate(moves: PgnMove[]): TrophyCheckResult {
  ...
  if (
    !lastMove.notation.fig &&
    lastMove.notation.col === 'g' &&
    lastMove.notation.row === '5' &&
    lastMove.notation.check === '#'
  ) {
    return [
      {
...
export function promotePawnBeforeMoveNumber(
  moves: PgnMove[],
  beforeMove: number,
): TrophyCheckResult {
